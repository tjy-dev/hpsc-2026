...
__device__ __forceinline__ void async_copy_packed_tile(const half *src, half *dst, int tid) {
...
__global__ void wgmma_kernel(int dim_m, int dim_n, int dim_k,
  ...
  __shared__ __align__(1024) half block_a[8192];
  __shared__ __align__(1024) half block_b[8192];
  ...
  uint64_t desc_a0 = make_wgmma_desc(block_a, 64, 1, 1);
  uint64_t desc_a1 = make_wgmma_desc(&block_a[4096], 64, 1, 1);
  uint64_t desc_b = make_wgmma_desc(block_b, 64, 1, 1);

  wgmma_fence();
  for (int kk = 0; kk < dim_k; kk += 16) {
    int ktile = kk / 16;
    const half *src_a = packed_a + (blockIdx.x * ktile_count + ktile) * 8192;
    const half *src_b = packed_b + (blockIdx.y * ktile_count + ktile) * 8192;
    async_copy_packed_tile(src_a, block_a, i);
    async_copy_packed_tile(src_b, block_b, i);
    cp_async_commit();
    cp_async_wait();
    __syncthreads();
    asm volatile("fence.proxy.async.shared::cta;\n" ::: "memory");
    wgmma_fence();
    wgmma_m64n128k16(desc_a0, desc_b, acc[0]);
    wgmma_m64n128k16(desc_a1, desc_b, acc[1]);
    wgmma_commit();
    wgmma_wait();
    __syncthreads();
  }
...
int main(int argc, const char **argv) {
  ...
  printf("CUBLAS: %.2f Gflops, WGMMA: %.2f Gflops\n", cublas_flops, wgmma_flops);
  double err = 0;
  for (int i=0; i<n; i++) {
    for (int j=0; j<m; j++) {
