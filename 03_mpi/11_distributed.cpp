...
int main(int argc, char** argv) {
  ...
  for(int i=0; i<N/size; i++) {
    ...
    ibody[i].fx = jbody[i].fx = buffer[i].fx = 0;
    ibody[i].fy = jbody[i].fy = buffer[i].fy = 0;
  }
  ...
  int s = N/size;
  MPI_Win win;
  MPI_Win_create(jbody, s*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win_fence(0, win);
    MPI_Put(buffer, s, MPI_BODY, send_to, 0, s, MPI_BODY, win);
    MPI_Win_fence(0, win);
    for(int i=0; i<N/size; i++) {
